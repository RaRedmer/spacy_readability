...
def validate_book(nlp, book):
    doc = nlp(book["text"])
    assert doc._.flesch_kincaid_grade_level == pytest.approx(book["flesch_kincaid_grade_level"], rel=1e-1)
    assert doc._.flesch_kincaid_reading_ease == pytest.approx(book["flesch_kincaid_reading_ease"], rel=1e-1)
    assert doc._.coleman_liau_index == pytest.approx(book["coleman_liau_index"], rel=1e-1)
    assert doc._.automated_readability_index == pytest.approx(book["automated_readability_index"], rel=1e-1)
    assert doc._.smog == pytest.approx(book["smog"], rel=1e-1)
    assert doc._.dale_chall == pytest.approx(book["dale_chall"], rel=1e-1)
    assert doc._.forcast == pytest.approx(book["forcast"], rel=1e-1)


def test_peter_rabbit(nlp):
    with open("tests/samples/peter_rabbit.json", encoding="utf-8") as fp:
        data = json.load(fp)
